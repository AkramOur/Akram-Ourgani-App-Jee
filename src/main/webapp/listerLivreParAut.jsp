...
List<Livre> lvs = lm.Allivres();
if(request.getParameter("aut") != null){
	
	lv = lm.livreParAut(lm.findLivreById(Integer.parseInt(request.getParameter("aut"))).getAuteur());
}
...
<select name = "aut"> 
         <option value="" disabled selected hidden>select auteur</option>
        <%
        for(Livre li : lvs) {
        %>
        <option value=<%=li.getId() %>><%=li.getAuteur() %></option>
        <%} %>
</select>
